...
For the purposes of this work, we denote $\Omega_a $($\Omega_m$) as the Rabi frequencies to drive a transition from the atom (target molecule) state to the intermediate state used in the Raman transition corresponding to the power in a single beam, $P_a$ ($P_m$). Thus, the Raman Rabi frequency remains $\Omega_a\Omega_m/2\Delta$. Cross coupling affects the scattering rate and light shift, and assuming equal powers in both beams, these quantities become $ \Gamma_e \Omega_m^2 / 2\Delta^2 + \Gamma_e \Omega_a^2 / 2\Delta^2 $ and $ \Omega_m^2/2\Delta - \Omega_a^2/2\Delta $, where $\Gamma_e$ is the excited state linewidth.  
