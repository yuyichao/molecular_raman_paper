...
nuclear spin character, which is unknown. These calculations include theoretical electronic transition dipole moments shown in figure \ref{tdm-sm} calculated with ab initio methods~\footnote{Private communication with R. Moszynski}.
...
\begin{figure}[ht!]
  \includegraphics[width=\textwidth]{imgs/TDM_wide.pdf}
  \caption{Electronic transition dipole moments calculated via ab initio methods between different electronic states of NaCs. These transition dipole moments are shown in units of $ea_0$, where $ e $ is the elementary charge and $a_0$ is the Bohr radius. They are plotted as a function of internuclear distance, $ R $.
    \label{tdm-sm}}
\end{figure}
