...
where the initial atomic state and the target weakly bound molecular state are coupled to an intermediate state by two lasers with Rabi frequencies, $\Omega_a$ and $\Omega_m$, with one-photon detuning $ \Delta $, and two-photon detuning, $ \delta$. Note that we use the convention where the Rabi frequency describes the population oscillation between the two states.  %(Ultimately, we calculate with contribution of all vibrational states of the excited electronic potential, $\mathrm{c^3\Sigma^+}$)
...
\begin{figure*}
  ...
  \caption{Raman transition parameters as a function of tweezer and Raman power and detuning.
    (a) The Raman resonance fitted to $a_P+b_P/\Delta$, where
    ...
    at zero tweezer power to be $\omega_{R0}=2\pi\times770.1969(2)~\mathrm{MHz}$.
...
We perform the experiment at various tweezer powers to extract the power dependence of $ \Omega_m $ and $ \Omega_a $, as shown in Fig.~\ref{f-det}c. $ \Omega_m $ scales with $ P^{0.5} $ as expected, and we obtain a fit value of $2\pi\times180.87(7)~\mathrm{MHz}/\sqrt{\mathrm{mW}}$. The scaling of $ \Omega_a $ differs from $ P^{0.5} $, which is the expected scaling from the laser intensity, due to the change in the atomic wavefunction caused by tighter confinement at higher power. For weakly interacting particles,
...
\begin{table}[ht]
  \centering
  \begin{tabular}{c|c}
    & Measured Values \\ \hline
    $\Omega_R $ & $2\pi \times 3.28(4)~\mathrm{kHz}$ \\
    $\Gamma_s$ & $2\pi \times 0.80(4)~\mathrm{kHz}$ \\
    $\Omega_m$ & $2\pi \times 348.3(3)~\mathrm{MHz} $  \\
    $\Omega_a$ & $2\pi \times 5.5(2)~\mathrm{MHz}$  \\
    $\Omega_m/\Omega_a$ & $63(3)$
  \end{tabular}
  \caption{Experimental values of $\Omega_R,\Gamma_s, \Omega_m, \Omega_a$. $\Omega_R$ and $ \Gamma_s $ are reported at $151~\mathrm{GHz}$ detuned from the $v' = 0$ state. All numbers are reported at $3.75$ mW tweezer power. }
