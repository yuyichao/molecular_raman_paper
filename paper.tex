...
In this idealized treatment, the ratio between the Raman Rabi frequency and the scattering rate is $ \Omega_{\rm a}/\Omega_{\rm m} \times \Delta/\Gamma_{\rm e} $, which limits the transfer efficiency into the molecular state. At the same time, the intensity-stability requirement is determined by the ratio of Raman Rabi frequency to light shift $\Omega_{\rm a}/\Omega_{\rm m}$. Notably, both figures of merit improve with a larger ratio $\Omega_{\rm a}/\Omega_{\rm m}$. %, depends on the ratio of the two matrix elements and how far detuned the laser is from the transition in units of the linewidth.
...
\TR{What is meant by ``result is consistent with the decay of the Rabi oscillation'' if independent fits not done?  See also Fig.~5 caption.}
