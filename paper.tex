...
\begin{abstract}
  ...
  The molecules have a binding energy of $770.1969(2)~\mathrm{MHz}$ \TR{SYSTEMATICS?  I doubt the optical power measurement is accurate enough to support light-shift corrections to 200~Hz accuracy.} at $8.83(2)~\mathrm{G}$
  and are created with higher than $60\mathrm{\%}$ probability\textcolor{blue}{(KK: this was not mentioned anywhere in the main text)} in the motional ground state.
...
and is chosen to minimize photon scattering during Raman Rabi \TR{Is Raman Rabi freq the best term?} oscillations.
...
The transfer Raman Rabi Rate is given by $\Omega_a\Omega_m / (2\Delta)$~\cite{Wineland2003}. \textcolor{blue}{[KK: is this the formula used in the citation?]}
...
which can be made smaller by using deeply bound vibrational states as the intermediate state \TR{Cite supplement or maybe Fig. S1 can go into Fig. 1b since it shows $\Gamma_s\sim \delta_{thresh}^{-2}$ nicely.}.
...
\begin{figure}[ht!]
  ...
  \caption{Coherent transfer of atoms to molecules.  The molecular state is dark to the imaging step and corresponds to zero signal.
    (a) Raman difference frequency scans at different times showing the resonance.
    ...
    The beam is focused to a waist of $0.9~\mathrm{\mu m}$
    that confines the atoms and molecule.
    A magnetic field of $B_0=8.83(2)~\mathrm{G}$ along the tweezer polarization
...
we use the tweezer beam itself with two frequencies, as shown in Fig.~\ref{f-beampath}.
The dual use of the tweezer beam for confinement and Raman transfer not only minimizes photon scattering,
but also allows a tight focus to minimize the transfer duration.
...
As shown in Fig.~\ref{f-beampath}, one filter immediately follows the laser, which is a fiber amplifer seeded with a $1037~\mathrm{nm}$ external cavity diode laser. The second filter precedes the microscope objective. \TR{Move some technical details to the figure caption.}
The filters reduce photon scattering rate by a factor of $2$.  While we have not fully characterized the sources 
...
\TR{``We" is overused in this paper and obscures more important subject-object relationships.  Authorship already implies that ``we" did everything in the paper.} We choose a tweezer frequency far detuned (by $-151~\mathrm{GHz}$) from the $v' = 0$ line to reduce resonant scattering. \textcolor{blue}{[KK: mention there are 3 lines at v'=0, cite our ~\cite{Liu2019}]}
Guided by the coupled-channel calculations,
we locate the Raman resonance as shown in Fig.~\ref{f-raman}, which shows a Fourier-limited resonance together with Rabi oscillations between the atomic and molecular states.
...
A decaying Rabi oscillation fitted to the data suggests that
$69~\mathrm{\%}$ of atoms initially in the motional ground state are transferred into the molecular state after a $\pi$ pulse.

\TR{combine with later} Raman transfer efficiency is limited mainly by the molecular lifetime,
which is measured directly by preparing the molecule with a $\pi $ pulse
...
We obtain the Raman Rabi frequency by fitting to our measurements a model
that includes a Raman Rabi frequency and
a finite lifetime for the molecular state, as shown in Fig.~\ref{f-raman}a and b.
We account for the small effect of atom loss by measuring the one-body and
...
of $2\pi\times3.28(4)~\mathrm{kHz}$.~\TR{Why does Fig. 5b show about 4 kHz?}
...
\begin{figure*}
  ...
  \caption{Raman transition parameters as a function of tweezer power and detuning.
    ...
    at zero tweezer power $\omega_{R0}=2\pi\times770.1969(2)~\mathrm{MHz}$.
    ...
    $c_P$ and $d_P$ are proportional to $P^{1.29}$
...
The $1/\Delta$ components, due to the nearby $v'=0$ intermediate state, determine $\Omega_m $ and $ \Omega_a $ in Table~\ref{tab:rabi_freqs} from the fit coefficients in Table \ref{tab:f-det:fit}.
...
Coupled-channel calculations show that the wave-function scaling
...
Furthermore, we independently measure the natural linewidth of the $v'=0$ excited state to be no larger than $50~\mathrm{MHz}$ using photoassociation (PA) spectroscopy.
...
\begin{table}[ht]
  ...
  \begin{tabular}{c|c|c|c|c}
    $\Omega_R$ & $\Gamma_s$ & $\Omega_m$ & $\Omega_a$ & $\Omega_a/\Omega_m$ \\ 
    ($2\pi\times$kHz) & ($2\pi\times$kHz) & ($2\pi\times$MHz) & ($2\pi\times$MHz) &  \\ \hline 
    X.YY & 0.80(4) & 348.3(3) & 5.5(2) & 63(3) \\
  \end{tabular}
  \caption{Experimentally determined coefficients at $3.75$~mW tweezer power. $\Omega_R$ and $ \Gamma_s $ are reported at $-151~\mathrm{GHz}$ detuning from the $v' = 0$ state.  The coefficients $\Omega_m$ and $\Omega_a$ are in broad agreement with our calculation and their ratio is near the theory prediction of $78$.  The measured Rabi rate $\Omega_R$ is ZZ~\% of $\Omega_m \Omega_a/2$ due to interference from further-detuned Raman processes. [[TR: Check all mentions of $P=3.75$~mW because they may negelect reduction from 2nd ASE filter.]]}
...
\TR{combine this and next paragraph with earlier} The efficiency of the transfer is limited by the short lifetime of the molecular state~(Fig.~\ref{f-lifetime}a) together with a reduction in the Raman Rabi frequency due to intermediate states beyond v'=0 (see Table~~\ref{tab:rabi_freqs}).  Preliminary experiments and theoretical considerations indicate that the molecular lifetime may be limited by two-photon coupling to the atomic continuum~\TR{Cite YY thesis.}. We also observe a decrease in the coherence time by a factor of 2 without laser spectrum filters,
suggesting that spectral impurity of the laser can be a significant source of scattering.
Other sources that can contribute to decoherence include
the stability of the tweezer power and magnetic field.
...
fluctuation of $\sim1.5~\mathrm{mG}$.~\TR{Mention somewhere that the shape of Fig. 3b Rabi flopping indicates molecular loss, not atomic loss or frequency fluctuation.}
...
in forming rovibrational ground state molecules. By scaling up to many optical tweezers~\cite{Endres2016, Barredo2018,  PhysRevLett.122.203601}, large arrays with arbitrary geometry of highly controlled molecules can be achieved.
