...
\begin{figure}[ht!]
  ...
  \caption{Coherent transfer of atoms to molecules.  The molecular state is dark to the imaging step and corresponds to zero signal.
    (a) Raman difference frequency scans for different durations showing the resonance.
    ...
    The beam X.yy~mW beam is focused to a waist of $0.9~\mathrm{\mu m}$
...
we modulate the tweezer beam with a second frequency, as shown in Fig.~\ref{f-beampath}.
...
\TR{``We" is overused in this paper and obscures more important subject-object relationships.  Authorship already implies that ``we" did everything in the paper.} We choose a tweezer frequency far detuned (by $-151~\mathrm{GHz}$) from the 3 observed rotational lines at $v' = 0 $ to reduce resonant scattering~\cite{Liu2019}.
...
$69~\mathrm{\%}$ of atoms initially in the relative motional ground state are transferred into the molecular state after a $\pi$ pulse. Over $60\%$ of the created molecules are in the motional ground state (both relative motion and center of mass motion), inferred from independent Raman sideband thermometry measurements of the atoms. A fit including atomic and molecular losses (see below) yields a Raman Rabi frequency of $2\pi\times3.28(4)~\mathrm{kHz}$.
...
\begin{table}[ht]
  ...
  \begin{tabular}{|c|c|c|c|}
    ...
    $\Omega_R$ ($2\pi\times$kHz) & & & X.YY \\
    $\Omega_m$ ($2\pi\times$MHz) & & & 348.3(3) \\
    $\Omega_a$ ($2\pi\times$kHz) & & & 5.5(2)
  \end{tabular}
