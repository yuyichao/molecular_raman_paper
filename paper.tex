...
While optical Raman transfer of atoms to molecules utilizing an electronic excited state has been shown previously\cite{Wynar2000,Rom2004}. Those demonstrations were  incoherent  where the signal shows resonance loss of population and the transferred molecules cannot be further utilized(need to improve). The challenge lies at  achieve high enough Raman Rabi frequency to drive this transition due to the wavefunction size mismatch with the excited molecular state.
...
Based on these considerations, we calculated both the Raman Rabi frequency and the scattering rate at different detunings (full result is shown in SM). As seen in figure (1....), despite a higher Raman Rabi frequency at the same detuning, using a weakly bound intermediate state results in a much higher scattering rate and ultimately a smaller Rabi frequency to scattering rate ratio as compared to deeply bound states. As a result, we use a deeply bound molecular excited state to drive the Raman transition. (mention the trend, also v=12 will work as similarly as v=0)
